...
export function enrollProcess(o: EnrollOpts): string {
  ...
  if (!o.tool) throw new Error("enroll: --tool required (where this process lives)");
  if (!o.metric) throw new Error("enroll: --metric required (the outcome it is accountable to)");
  const autonomy = o.autonomy ?? "shadow"; // autonomy is earned; everything starts in shadow
  if (!AUTONOMY.includes(autonomy)) throw new Error(`enroll: --autonomy must be one of ${AUTONOMY.join("|")}`);
  const policy: Record<string, unknown> = {};
  if (o.guardrails?.length) policy.guardrails = o.guardrails;
  if (o.blastRadius) policy.blast_radius = o.blastRadius;
  const db = openDb();
  try {
    db.prepare(
      "INSERT INTO processes (id, tool, metric, inverse, autonomy, status, policy, created_at) VALUES (?,?,?,?,?, 'running', ?, ?)"
    ).run(o.id, o.tool, o.metric, o.inverse ? 1 : 0, autonomy, JSON.stringify(policy), Date.now());
  } catch (e) {
...
export function startExperimentDetailed(processId: string, field: string, control: string, variant: string, days = 7, status: "running" | "launching" = "running", share?: number, object?: string, baseline?: number, baselineUnit?: string, baselineInverse?: boolean, valueBasis?: number, power?: unknown): { id: string; message: string } {
  ...
  const running = db.prepare(
    "SELECT id FROM experiments WHERE process_id = ? AND status IN ('running','launching')"
  ).get(processId) as ExperimentRow | undefined;
  if (running) throw new Error(`start: ${processId} already has a running experiment (${running.id}) — one at a time`);
