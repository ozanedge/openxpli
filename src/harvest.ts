...
import { openDb, type ExperimentRow, type ProcessRow, type HoldoutRow } from "./db.js";
...
import {
  ...
  HOLDOUT_SHARE, VALIDATION_HOURS, REGRESS_WINDOW_HOURS,
} from "./paths.js";
...
export function harvest(now = Date.now()): void {
  ensureDirs();
  ...
  let filled = 0, gaps = 0, finalized = 0, errors: string[] = [];
  ...
  try {
    ...
    const unledgered = db.prepare(
      "SELECT * FROM experiments WHERE status IN ('won','failed') AND record_id IS NULL"
    ).all() as ExperimentRow[];
    for (const exp of unledgered) {
      const proc = db.prepare("SELECT * FROM processes WHERE id = ?").get(exp.process_id) as ProcessRow;
      const counts = db.prepare(
        "SELECT SUM(missing = 0) AS ok, SUM(missing = 1) AS miss FROM observations WHERE experiment_id = ? AND phase = 'run'"
      ).get(exp.id) as { ok: number; miss: number };
      ...
      if (exp.status === "won") {
        db.prepare(
          "INSERT OR IGNORE INTO holdouts (experiment_id, process_id, started_at, ends_at, share, status) VALUES (?,?,?,?,?, 'validating')"
        ).run(exp.id, exp.process_id, exp.ends_at, exp.ends_at + VALIDATION_HOURS * HOUR_MS, HOLDOUT_SHARE);
      }
    ...
    for (const exp of exps) {
      ...
      const have = new Set(
        (db.prepare("SELECT hour FROM observations WHERE experiment_id = ?").all(exp.id) as { hour: number }[]).map(r => r.hour)
      );
      const insert = db.prepare(
        "INSERT OR IGNORE INTO observations (experiment_id, hour, ts, multiple, sigma, source, missing) VALUES (?,?,?,?,?,?,?)"
      );

      for (let hour = 1; hour <= dueHours; hour++) {
        if (have.has(hour)) continue;
      ...
      if (now >= exp.ends_at) {
        const last = db.prepare(
          "SELECT multiple FROM observations WHERE experiment_id = ? AND missing = 0 ORDER BY hour DESC LIMIT 1"
        ).get(exp.id) as { multiple: number } | undefined;
        const finalMultiple = last?.multiple ?? 1.0;
        const status = finalMultiple > 1.0 ? "won" : "failed";
        const counts = db.prepare(
          "SELECT SUM(missing = 0) AS ok, SUM(missing = 1) AS miss FROM observations WHERE experiment_id = ?"
        ).get(exp.id) as { ok: number; miss: number };
    ...
    const holdouts = db.prepare("SELECT * FROM holdouts WHERE status = 'validating'").all() as HoldoutRow[];
    for (const h of holdouts) {
      const exp = db.prepare("SELECT * FROM experiments WHERE id = ?").get(h.experiment_id) as ExperimentRow;
      const procH = db.prepare("SELECT * FROM processes WHERE id = ?").get(h.process_id) as ProcessRow;
      ...
      const have = new Set(
        (db.prepare("SELECT hour FROM observations WHERE experiment_id = ? AND phase = 'holdout'").all(exp.id) as { hour: number }[])
          .map((r) => r.hour - hourOffset)
      );
      const insert = db.prepare(
        "INSERT OR IGNORE INTO observations (experiment_id, hour, ts, multiple, sigma, source, missing, phase) VALUES (?,?,?,?,?,?,?, 'holdout')"
      );
      for (let hour = 1; hour <= dueHours; hour++) {
        if (have.has(hour)) continue;
        if (realH) {
          ...
          if (hour < dueHours) { insert.run(exp.id, hourOffset + hour, h.started_at + hour * HOUR_MS, null, null, "browser", 1); gaps++; }
          continue;
        }
      ...
      const trail = db.prepare(
        "SELECT AVG(multiple) m, COUNT(*) n FROM (SELECT multiple FROM observations WHERE experiment_id = ? AND phase = 'holdout' AND missing = 0 ORDER BY hour DESC LIMIT ?)"
      ).get(exp.id, REGRESS_WINDOW_HOURS) as { m: number | null; n: number };
      ...
      if (sustainedDrop || windowDone) {
        const last = db.prepare(
          "SELECT multiple FROM observations WHERE experiment_id = ? AND phase = 'holdout' AND missing = 0 ORDER BY hour DESC LIMIT 1"
        ).get(exp.id) as { multiple: number } | undefined;
