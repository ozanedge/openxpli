...
export function deleteConnector(id: string, confirmed: boolean): string {
  if (typeof id !== "string" || !id.trim()) throw new Error("Choose a connector to delete");
  if (confirmed !== true) throw new Error("Confirm connector deletion first");
  activeLearningJob(id); // Recover interrupted workers before checking for active work.
  const db = openDb();
  try {
    return db.transaction(() => {
      if (!db.prepare("SELECT 1 FROM processes WHERE id = ?").get(id)) throw new Error("No such connector");
