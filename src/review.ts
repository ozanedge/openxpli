...
function mutateRecord(recordId: string, newStatus: string, note: string, commitMsg: string): void {
  const path = join(LEDGER_DIR, `${recordId}.md`);
  if (!existsSync(path)) throw new Error(`no such record: ${recordId}`);
...
function expForRecord(recordId: string): ExperimentRow {
  const db = openDb();
  const exp = db.prepare("SELECT * FROM experiments WHERE record_id = ?").get(recordId) as ExperimentRow | undefined;
  if (!exp) throw new Error(`no experiment linked to ${recordId}`);
  return exp;
}
...
function assertOpen(experimentId: string, recordId: string): void {
  const o = outcomeFor(experimentId);
  if (!o) return; // pre-outcomes record; the file remains the only source
  if (o.review_state !== "open" && o.review_state !== "reopened")
    throw new Error(`${recordId} is not open for review (${o.review_state})`);
}

export function approve(recordId: string): string {
...
export function reject(recordId: string): string {
  const exp = expForRecord(recordId);
  assertOpen(exp.id, recordId);
  const db = openDb();
...
export function extend(experimentId: string, days: number): string {
  if (!(days > 0 && days <= 28)) throw new Error("extend: days must be 1..28");
  const db = openDb();
  const exp = db.prepare("SELECT * FROM experiments WHERE id = ?").get(experimentId) as ExperimentRow | undefined;
  if (!exp) throw new Error(`no such experiment: ${experimentId}`);
  ...
  if (!exp.record_id) throw new Error(`${experimentId} is ${exp.status} with no record`);
  const db2 = openDb();
...
export function kill(experimentId: string): string {
  const db = openDb();
  const exp = db.prepare("SELECT * FROM experiments WHERE id = ?").get(experimentId) as ExperimentRow | undefined;
  if (!exp) throw new Error(`no such experiment: ${experimentId}`);
  if (exp.status !== "running") throw new Error(`${experimentId} is not running`);
  const last = db.prepare(
    "SELECT multiple FROM observations WHERE experiment_id = ? AND missing = 0 ORDER BY hour DESC LIMIT 1"
  ).get(experimentId) as { multiple: number } | undefined;
