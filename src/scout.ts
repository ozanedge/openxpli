...
import { assess, explain, WEEK_HOURS, type Volume } from "./power.js";
...
import { BrowserCancelled, type BrowserWait } from "./browser-session.js";
...
// accepts a candidate to prepare a manual kit; no experiment is started.
...
export function insertCandidates(sourceId: string, rows_: { field: string; control_value: string; variant_value: string; metric: string; inverse: number | boolean; rationale: string; expected_multiple: number; volume?: Volume; arms?: number; evidence?: { url: string; text: string; observedAt: number }[] }[], replaceProposed = false): CandidateRow[] {
  let rows = rows_;
  ...
  if (!replaceProposed && proposed >= 3) return listCandidates(sourceId); // a scout already landed; don't stack or clobber
  ...
  const powers = new Map<string, ReturnType<typeof assess>>();
  rows = rows.filter((r) => {
    if (!r.volume) return true; // no volume read for this object; cannot judge
    const arms = r.arms ?? 3;
  ...
  const ins = db.prepare(
    "INSERT OR IGNORE INTO candidates (id, source_id, field, control_value, variant_value, metric, inverse, rationale, expected_multiple, status, created_at, power, run_hours, evidence) VALUES (?,?,?,?,?,?,?,?,?, 'proposed', ?,?,?,?)"
  );
  db.transaction(() => {
    if (replaceProposed) db.prepare("UPDATE candidates SET status = 'dismissed' WHERE source_id = ? AND status = 'proposed'").run(sourceId);
    rows.forEach((r, n) => {
      const pw = powers.get(r.field) ?? null;
      ins.run(`${short}#${used + n + 1}`, sourceId, r.field, r.control_value, r.variant_value, r.metric,
      r.inverse ? 1 : 0, r.rationale,
      r.volume ? r.expected_multiple : Math.min(1.2, Math.max(1.005, r.expected_multiple || 1.03)),
      Date.now(), pw ? JSON.stringify(pw) : null, pw ? pw.run_hours : null, r.evidence ? JSON.stringify(r.evidence) : null);
    });
  }).immediate();
...
export async function rescout(sourceId: string, _isChild = false, options: BrowserWait = {}): Promise<CandidateRow[]> {
  ensureCandidatesTable();
  const db = openDb();
  try {
    ...
    if (!pb) return scout(sourceId);
    if (!existsSync(BROWSER_PROFILE)) throw new Error("NEED_SIGNIN");
...
export function maybeAnalyze(): void {
  ...
  for (const r of empty) {
    if (playbookFor(r.tool)) {
      ...
      if (attempted) continue; // retries belong to the user, not every polling request
      if (existsSync(BROWSER_PROFILE)) spawnDetachedScout(r.id);
      else learningStatus(r.id, "needs-signin", "Sign in to read the account and prepare grounded suggestions.");
    } else scout(r.id);
  }
}
