...
export function maybeAnalyze(): void {
  ...
  const empty = db.prepare(`SELECT p.id FROM processes p WHERE p.status = 'proposed'
    ...
    AND NOT EXISTS (SELECT 1 FROM experiments e WHERE e.process_id = p.id AND e.status = 'running')`).all() as { id: string }[];
  for (const r of empty) { db.prepare("UPDATE processes SET status = 'shadow', created_at = ? WHERE id = ?").run(Date.now() - ANALYZE_MS, r.id); scout(r.id); }
}
