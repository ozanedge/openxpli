...
export interface Power {
  arms: number;
  run_hours: number;
  clicks_per_arm: number;
  impressions_per_arm: number;
  mde: number;                 // smallest relative difference the run can distinguish
  expected: number;            // the relative effect the experiment expects
  resolvable: boolean;
  needed_hours: number | null; // whole weeks required, null if beyond MAX_RUN_WEEKS
  basis: string;
}
...
function relSE(kind: "cost-per-click" | "rate", vol: Volume, days: number, arms: number): number {
...
const mdeFrom = (se: number) => 1.96 * se * Math.SQRT2;

export function assess(
  metric: string,
  inverse: number | boolean,
  vol: Volume,
  arms: number,
  expectedMultiple: number,
  runHours = WEEK_HOURS
): Power {
  const kind: "cost-per-click" | "rate" = /cost per click|cpc|cost per/i.test(metric) ? "cost-per-click" : "rate";
...
export function explain(p: Power): string {
