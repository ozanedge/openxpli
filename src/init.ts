...
export function init(): void {
  ...
  const installAgent = (label: string, args: string[], schedule: string): void => {
...
`;
    ...
    try { execFileSync("launchctl", ["bootout", target], { stdio: "ignore" }); } catch { /* not loaded yet */ }
    for (let attempt = 0; ; attempt++) {
      try { execFileSync("launchctl", ["bootstrap", `gui/${uid}`, plistPath], { stdio: "ignore" }); break; }
      catch (e) {
