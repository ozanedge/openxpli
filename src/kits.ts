...
import type { CandidateRow } from "./scout.js";
import { ratifiedGoal } from "./goals.js";
import { ask, jsonFrom } from "./browser-scout.js";

export interface KitField { label: string; value: string | null; source: string | null; }
...
export interface Kit {
  ...
  process_id: string;
  goal_id: string;
  state: "queued" | "preparing" | "ready" | "needs-input" | "failed" | "launched";
...
export function getKit(id: string): Kit {
  const db = openDb();
  try {
    const row = db.prepare(`SELECT ${KIT_COLUMNS} FROM kits WHERE id = ?`).get(id) as Record<string, unknown> | undefined;
    if (!row) throw new Error("No such experiment kit");
    return unpack(row);
  } finally { db.close(); }
}
export function listKits(processId: string): Kit[] {
  const db = openDb();
  try {
    ...
    return (db.prepare(`SELECT ${KIT_COLUMNS} FROM kits WHERE process_id = ? ORDER BY created_at DESC`).all(processId) as Record<string, unknown>[]).map(unpack);
  } finally { db.close(); }
}
export function requestKit(candidateId: string, launchWorker = true): Kit {
  const db = openDb();
  try {
    ...
    const existing = db.prepare("SELECT id FROM kits WHERE candidate_id = ?").get(candidateId) as { id: string } | undefined;
    if (existing) return getKit(existing.id);
    if (!c.evidence || !JSON.parse(c.evidence).length) throw new Error("Learn this account through the browser first (sign in, then request new suggestions). Template suggestions cannot produce an account-specific kit.");
    if (c.status !== "proposed") throw new Error("Choose a proposed candidate to prepare a kit");
    const goal = ratifiedGoal(c.source_id);
    if (!goal || c.metric !== goal.metric || c.inverse !== goal.inverse) throw new Error("This suggestion no longer matches the current goal. Scout fresh suggestions first.");
    const id = randomUUID();
    ...
    const saved = db.prepare("SELECT id FROM kits WHERE candidate_id = ?").get(candidateId) as { id: string };
    if (saved.id === id && launchWorker) startWorker(id);
    return getKit(saved.id);
  } finally { db.close(); }
}
function startWorker(id: string): void {
  ...
  child.on("error", () => {
    const db = openDb();
    try { db.prepare("UPDATE kits SET state = 'failed', error = 'Could not start preparation worker' WHERE id = ? AND state = 'queued'").run(id); }
    finally { db.close(); }
  });
  child.unref();
}
export function retryKit(id: string, launchWorker = true): Kit {
  const db = openDb();
  try {
    ...
    if (result.changes && launchWorker) startWorker(id);
    return getKit(id);
  } finally { db.close(); }
}
function string(value: unknown, name: string, max = 4000): string {
  if (typeof value !== "string" || !value.trim() || value.length > max) throw new Error(`Invalid ${name} in prepared kit`);
  return value.trim();
}
export function pngSize(image: Buffer): string {
  ...
  if (!width || !height || width * height > 40_000_000) throw new Error("Invalid PNG dimensions");
  return `${width} × ${height} px`;
}
export async function generateKitImage(prompt: string): Promise<Buffer> {
  const key = process.env.OPENAI_API_KEY;
  if (!key) throw new Error("Image generation is not configured. Attach a PNG, or configure OPENAI_API_KEY on the server and retry.");
  ...
  if (!response.ok) throw new Error(`Image generation failed (HTTP ${response.status}). Check the image provider configuration and retry.`);
...
export function assembleKit(c: CandidateRow, policy: unknown, raw: unknown): KitContent {
  ...
  for (const label of ["headline", "description", "CTA", "destination URL"]) {
    if (canonical(label) !== canonical(c.field) && !unchanged.some((f) => canonical(f.label) === canonical(label))) unchanged.push({ label, value: null, source: null });
  }
...
export async function buildKit(id: string, providers = { text: ask, image: generateKitImage }): Promise<Kit> {
  const db = openDb();
  const claimed = db.prepare("UPDATE kits SET state = 'preparing', updated_at = ? WHERE id = ? AND state = 'queued'").run(Date.now(), id);
  if (!claimed.changes) { db.close(); return getKit(id); }
  try {
    let kit = getKit(id);
    const c = db.prepare("SELECT * FROM candidates WHERE id = ?").get(kit.candidate_id) as CandidateRow;
    const proc = db.prepare("SELECT * FROM processes WHERE id = ?").get(kit.process_id) as ProcessRow;
    if (!kit.content) {
...
export function attachKitImage(id: string, image: Buffer): Kit {
  const size = pngSize(image), kit = getKit(id);
  if (!kit.content || !["ready", "needs-input"].includes(kit.state)) throw new Error("Wait for the brief to finish before attaching an image");
  const db = openDb();
  try {
...
export function recordKitLaunch(id: string, note: string, confirmed: boolean): Kit {
  ...
  if (kit.state !== "ready" || !kit.content || confirmed !== true) throw new Error("Review the kit and confirm you launched it yourself first");
  const goal = ratifiedGoal(kit.process_id);
  if (goal?.id !== kit.goal_id) throw new Error("The goal changed since this kit was prepared. Prepare a new kit against the current goal.");
...
export function kitImage(id: string): Buffer | null {
  const db = openDb();
  try { return (db.prepare("SELECT image FROM kits WHERE id = ?").get(id) as { image: Buffer | null } | undefined)?.image ?? null; }
  finally { db.close(); }
}
export function kitText(kit: Kit): string {
  const c = kit.content;
  if (!c) throw new Error("This kit is still being prepared");
...
export function kitHtml(kit: Kit): string {
  const c = kit.content;
  if (!c) throw new Error("This kit is still being prepared");
