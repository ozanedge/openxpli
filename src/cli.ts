...
switch (cmd) {
  ...
  case "rescout": {
    ...
    try {
      if (!process.argv[3]) throw new Error("usage: openxpli rescout <connector>");
  ...
  case "signin": {
    ...
    try {
      ...
      if (!url && arg) {
        const p = db.prepare("SELECT tool FROM processes WHERE id = ?").get(arg) as { tool: string } | undefined;
        const pb = p && playbookFor(p.tool);
        if (pb) { url = pb.url; connector = arg; }
      }
      if (!url) throw new Error("usage: openxpli signin <connector-id|url>");
      await signin(url);
