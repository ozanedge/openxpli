...
async function enrollDemo(): Promise<void> {
  ...
  db.prepare(
    ...
    "openxpli/readme-quickstart", "GitHub", "install conversion", 0, "human-gated", "running",
    JSON.stringify({ guardrails: ["doc engagement must not drop"], blast_radius: "README only" }), now
  );
  const ins = db.prepare(
    "INSERT OR IGNORE INTO experiments (id, process_id, field, control_value, variant_value, started_at, ends_at, status) VALUES (?,?,?,?,?,?,?, 'running')"
  );
  ...
  ins.run("readme-quickstart/081926", "openxpli/readme-quickstart", "quickstart heading",
    "Getting started", "Run your first experiment in 5 minutes", doneStart, doneStart + DEFAULT_RUN_HOURS * HOUR_MS);
...
switch (cmd) {
  ...
  case "start": {
    ...
    const flag = (n: string) => { const i = process.argv.indexOf(`--${n}`); return i > -1 ? process.argv[i + 1] : undefined; };
    try {
      if (!process.argv[3] || process.argv[3].startsWith("--")) throw new Error("usage: openxpli start <process-id> --field X --control A --variant B [--days N]");
