...
async function enrollDemo(): Promise<void> {
  ...
  db.prepare(
    "INSERT OR IGNORE INTO processes (id, tool, metric, inverse, autonomy, status, policy, created_at) VALUES (?,?,?,?,?,?,?,?)"
  ).run(id, "ChatGPT Ads", "cost per acquisition", 1, "human-gated", "running",
    JSON.stringify({ guardrails: ["conversion volume must not drop", "daily spend must not rise"], blast_radius: "$250/day spend cap" }), now);

  const { ratifiedGoal, adoptGoal } = await import("./goals.js");
  const gid = ratifiedGoal(id)?.id
    ?? adoptGoal(id, "cost per acquisition", 1,
      ["conversion volume must not drop", "daily spend must not rise"],
  ...
  const ins = db.prepare(
    `INSERT OR IGNORE INTO experiments (id, process_id, field, control_value, variant_value, started_at, ends_at,
  ...
  ins.run("main-account/0819", id, "creative format", "static image", "short motion loop",
    doneStart, doneStart + DEFAULT_RUN_HOURS * HOUR_MS, gid, 0.4, "creative format", "static image", 0.2);
  ...
  ins.run("main-account/0825", id, "bid", "current CPC bid", "bid -15% with dayparting",
    liveStart, liveStart + DEFAULT_RUN_HOURS * HOUR_MS, gid, 0.4, "bid", "current CPC bid", 0.2);
...
function status(): void {
  const db = openDb();
  const procs = (db.prepare("SELECT COUNT(*) c FROM processes").get() as { c: number }).c;
  const byStatus = db.prepare("SELECT status, COUNT(*) c FROM experiments GROUP BY status").all() as { status: string; c: number }[];
  const obs = (db.prepare("SELECT COUNT(*) c FROM observations").get() as { c: number }).c;
  const holdouts = db.prepare("SELECT status, COUNT(*) c FROM holdouts GROUP BY status").all() as { status: string; c: number }[];
  ...
  if (existsSync(HEARTBEAT_PATH)) console.log(`heartbeat: ${readFileSync(HEARTBEAT_PATH, "utf8").trim()}`);
}
...
switch (cmd) {
  case "browser": {
    ...
    if (BROWSER_MODE !== "attach") { console.log('browser: OPENXPLI_BROWSER_MODE=launch — no shared browser is used. Unset it to use one.'); break; }
    if (process.argv.includes("--import-profile")) {
      ...
      const busy = (pattern: string) => { try { return execSync(`pgrep -f ${JSON.stringify(pattern)} | head -1`, { encoding: "utf8" }).trim(); } catch { return ""; } };
      if (busy(`user-data-dir=${BROWSER_PROFILE}`)) throw new Error('The OpenXPLI browser is open on the destination. Quit it, then run this again.');
      const userData = chromeUserDataDir();
      const profile = chromeProfileDir(userData);
      if (busy("MacOS/Google Chrome")) console.log("note: Chrome is running — quitting it first gives a cleaner copy of the cookie store.");
    ...
    if (process.argv.includes("--reset-checks")) {
      ...
      if (!pb) throw new Error("no playbook to reset");
      await startChrome(false); await ensureTarget();
  ...
  case "browser-job": {
    ...
    try { await runLearningJob(process.argv[3]); }
  ...
  case "kit-build": {
    ...
    try {
      ...
      if (kit.state === "failed" || kit.state === "needs-input") process.exitCode = 1;
    } catch (e) { console.error(String(e)); process.exitCode = 1; }
    break;
  }
  case "prepare": {
    ...
    try {
      if (!process.argv[3]) throw new Error("usage: openxpli prepare <candidate-id>");
  ...
  case "plan": {
    ...
    try {
      if (!expId) throw new Error("usage: openxpli plan <experiment-id>");
      const row = openDb().prepare(
        "SELECT e.*, p.tool FROM experiments e JOIN processes p ON p.id = e.process_id WHERE e.id = ?"
      ).get(expId) as Record<string, unknown> | undefined;
      if (!row) throw new Error(`plan: no such experiment: ${expId}`);
      const pb = pbf(row.tool as string);
      if (!pb) throw new Error(`plan: no playbook for ${row.tool}`);
      console.log(await act(row as never, pb, "plan"));
    } catch (e) { console.error(String(e instanceof Error ? e.message : e)); process.exitCode = 1; }
    break;
  }
  case "activate": {
    ...
    const expId = process.argv[3];
    try {
      if (!expId) throw new Error("usage: openxpli activate <experiment-id>");
      const db = openDb();
      const row = db.prepare(
        "SELECT e.*, p.tool, p.autonomy FROM experiments e JOIN processes p ON p.id = e.process_id WHERE e.id = ?"
      ).get(expId) as Record<string, unknown> | undefined;
      if (!row) throw new Error(`activate: no such experiment: ${expId}`);
      if (row.status !== "launching") throw new Error(`activate: ${expId} is ${row.status}, not awaiting activation`);
      const pb = pbf(row.tool as string);
      if (!pb) throw new Error(`activate: no playbook for ${row.tool}`);
      console.log(await act(row as never, pb, "apply", false));
  ...
  case "recipe": {
    ...
    const expId = process.argv[3];
    try {
      if (!expId) throw new Error("usage: openxpli recipe <experiment-id>");
      await makeRecipe(expId);
    } catch (e) { console.error(String(e instanceof Error ? e.message : e)); process.exitCode = 1; }
    finally { if (expId && process.argv.includes("--child")) clearLock(`recipe-${expId.replace(/[^a-z0-9]/gi, "-")}.lock`); }
    break;
  }
  ...
  case "rescout": case "signin": {
    ...
    try {
      ...
      if (!sourceId) throw new Error(`usage: openxpli ${cmd} <connector-id>`);
      if (cmd === "signin" && /^https?:/.test(sourceId)) await signin(playbookForUrl(sourceId));
      else {
  ...
  case "act": {
    ...
    const expId = process.argv[3];
    try {
      if (!expId) throw new Error("usage: openxpli act <experiment-id>");
      const exp = db.prepare(
        "SELECT e.*, p.tool FROM experiments e JOIN processes p ON p.id = e.process_id WHERE e.id = ?"
      ).get(expId) as { id: string; field: string; control_value: string; variant_value: string; tool: string; status: string } | undefined;
      if (!exp) throw new Error(`no such experiment: ${expId}`);
      const pb = pbFor(exp.tool);
      if (!pb) throw new Error(`no playbook for ${exp.tool}`);
      ...
      if (expId) db.prepare("UPDATE experiments SET launch_note = ? WHERE id = ?").run(`launch failed: ${msg}`, expId);
      console.error(msg);
      process.exitCode = 1;
    }
    break;
  }
  case "accept": case "dismiss": {
    const { acceptCandidate, dismissCandidate } = await import("./scout.js");
    try {
      if (!process.argv[3]) throw new Error(`usage: openxpli ${cmd} <candidate-id>`);
  ...
  case "start": {
    ...
    const flag = (n: string) => { const i = process.argv.indexOf(`--${n}`); return i > -1 ? process.argv[i + 1] : undefined; };
    try {
      if (!process.argv[3] || process.argv[3].startsWith("--")) throw new Error("usage: openxpli start <process-id> --field X --control A --variant B [--on \"ad name\"] [--days N] [--share 0.5]");
      console.log(startExperiment(process.argv[3], flag("field") ?? "", flag("control") ?? "", flag("variant") ?? "",
        Number(flag("days") ?? 7), "running", flag("share") != null ? Number(flag("share")) : undefined, flag("on"),
        flag("baseline") != null ? Number(flag("baseline")) : undefined, flag("unit")));
    } catch (e) { console.error(String(e instanceof Error ? e.message : e)); process.exitCode = 1; }
    break;
  }
  case "autonomy": {
    ...
    try {
      if (!process.argv[3] || li < 0) throw new Error("usage: openxpli autonomy <connector> --level human-gated|auto-start|auto-merge");
      console.log(setAutonomy(process.argv[3], process.argv[li + 1]));
    } catch (e) { console.error(String(e instanceof Error ? e.message : e)); process.exitCode = 1; }
    break;
  }
  case "approve": case "reject": case "extend": case "kill": {
    const { approve, reject, extend, kill } = await import("./review.js");
    const arg = process.argv[3];
    try {
      if (!arg) throw new Error(`usage: openxpli ${cmd} <${cmd === "approve" || cmd === "reject" ? "record-id" : "experiment-id"}>${cmd === "extend" ? " [--days N]" : ""}`);
      if (cmd === "approve") console.log(approve(arg));
      else if (cmd === "reject") console.log(reject(arg));
      else if (cmd === "kill") console.log(kill(arg));
      else {
