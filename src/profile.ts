...
export function chromeProfileDir(userData = chromeUserDataDir()): string {
  if (process.env.OPENXPLI_CHROME_PROFILE) return process.env.OPENXPLI_CHROME_PROFILE;
  try {
    const state = JSON.parse(readFileSync(join(userData, "Local State"), "utf8")) as { profile?: { last_used?: string } };
    const last = state.profile?.last_used;
    if (last && existsSync(join(userData, last))) return last;
  } catch { /* fall through to the default profile */ }
  return "Default";
}

export function profileNames(userData = chromeUserDataDir()): string[] {
  try {
    const state = JSON.parse(readFileSync(join(userData, "Local State"), "utf8")) as { profile?: { info_cache?: Record<string, unknown> } };
...
export interface ImportResult { from: string; profile: string; to: string; bytes: number; }
...
export function importChromeProfile(opts: { userData?: string; profile?: string; force?: boolean } = {}): ImportResult {
  ...
  if (!existsSync(source)) throw new Error(`No Chrome profile at ${source}. Profiles found: ${profileNames(userData).join(", ") || "none"}. Set OPENXPLI_CHROME_PROFILE to pick one.`);
  if (existsSync(IMPORT_MARKER) && !opts.force) throw new Error(`${BROWSER_PROFILE} was already imported. Pass --force to replace it (any sign-ins made only in the OpenXPLI browser are lost).`);
  ...
  if (run.status !== 0) throw new Error(`Copying the profile failed: ${(run.stderr || run.error?.message || "unknown error").slice(0, 300)}`);
  const state = spawnSync("cp", [join(userData, "Local State"), join(BROWSER_PROFILE, "Local State")], { encoding: "utf8" });
  if (state.status !== 0) throw new Error(`Copying Local State failed: ${(state.stderr || "unknown error").slice(0, 300)}`);
