...
export function doctor() {
    ...
    try {
        const db = openDb();
        const p = db.prepare("SELECT COUNT(*) c FROM processes").get().c;
        const e = db.prepare("SELECT COUNT(*) c FROM experiments WHERE status='running'").get().c;
        const gaps = db.prepare("SELECT COUNT(*) c FROM observations WHERE missing=1").get().c;
