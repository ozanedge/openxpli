import { readAdRows } from "./kit-evidence.js";
...
export function claudeBinary() {
    ...
    if (override) {
        if (!existsSync(override))
            throw new Error(`OPENXPLI_CLAUDE points at ${override}, which does not exist.`);
        return override;
    }
    if (claudeBin)
        return claudeBin;
...
export function ask(prompt, timeoutMs = 240_000) {
    return new Promise((resolve, reject) => {
        ...
        const child = spawn(bin, ["-p", prompt, "--model", "claude-opus-5", "--tools", "", "--strict-mcp-config", "--mcp-config", "{\"mcpServers\":{}}"], { stdio: ["ignore", "pipe", "pipe"], env: { ...process.env, PATH: RICH_PATH } });
        let out = "", err = "", done = false;
        const finish = (error) => { if (done)
            return; done = true; clearTimeout(timer); error ? reject(error) : resolve(out); };
        ...
        child.stdout.on("data", (d) => { out += d; if (out.length > 8e6) {
            child.kill("SIGKILL");
            finish(new Error("Text generation returned more output than expected."));
        } });
