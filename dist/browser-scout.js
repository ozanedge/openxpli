...
export function playbookFor(tool) {
    ...
    if (/openai|chatgpt ads/.test(t))
        return { url: process.env.OPENXPLI_SCOUT_URL ?? "https://ads.openai.com", name: "ads.openai.com" };
    if (process.env.OPENXPLI_SCOUT_URL)
        return { url: process.env.OPENXPLI_SCOUT_URL, name: "override" };
    return null;
}
async function openCtx(headless) {
...
// ── model helper ──
function ask(prompt, timeoutMs = 240_000) {
    return new Promise((resolve, reject) => {
        execFile("claude", ["-p", prompt, "--model", "claude-opus-5"], { timeout: timeoutMs, maxBuffer: 4 * 1024 * 1024 }, (err, stdout) => err ? reject(new Error(`model call failed: ${String(err).slice(0, 200)}`)) : resolve(stdout));
    });
}
function jsonFrom(raw, opener) {
...
export async function crawl(pb, maxPages = 8) {
...
export async function learn(sourceId, toolName, pb) {
...
export async function act(exp, pb) {
