...
export function endpoint() { return `http://127.0.0.1:${CHROME_PORT}`; }
export async function chromeAlive(timeoutMs = 2_000) {
    try {
        return (await fetch(`${endpoint()}/json/version`, { signal: AbortSignal.timeout(timeoutMs) })).ok;
    }
    catch {
        return false;
    }
}
export async function pageTargets(timeoutMs = 3_000) {
    try {
...
export async function ensureTarget(waitMs = 6_000) {
    if ((await pageTargets()).length)
        return;
...
export function flagsFor(headless) {
...
export function runningMode() {
    try {
        ...
        for (const pid of pids) {
            ...
            if (!cmd.includes("MacOS/Google Chrome") || cmd.includes("--type="))
                continue;
...
export async function stopChrome(waitMs = 8_000) {
    const pids = (spawnSync("pgrep", ["-f", `user-data-dir=${BROWSER_PROFILE}`], { encoding: "utf8" }).stdout ?? "").trim().split(/\s+/).filter(Boolean);
    if (!pids.length)
        return false;
    for (const pid of pids) {
    ...
    const deadline = Date.now() + waitMs;
    while (Date.now() < deadline) {
        if (!(await chromeAlive(800)) && runningMode() === null)
            return true;
...
export async function startChrome(headless = false, waitMs = 25_000) {
    const mode = headless ? "headless" : "headed";
    const running = runningMode();
    if (running === mode && await chromeAlive())
        return;
    // One profile directory, one Chrome. Swapping modes means replacing it.
    if (running && running !== mode)
        await stopChrome();
    else if (await chromeAlive())
        return;
    if (!existsSync(CHROME_BINARY))
        throw new Error(`Chrome was not found at ${CHROME_BINARY}. Set OPENXPLI_CHROME to its path.`);
...
export async function attachSession(headless = true) {
